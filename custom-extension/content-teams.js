...
function teams() {
  ...
  const extensionStatusJSON_bug = {
    "status": 400,
    "message": `<strong>ReadAi Meeting Scraper encountered a new error</strong> <br /> Please report it <a href="https://github.com/bhawani-nexus/readai-meeting-scraper/issues" target="_blank">here</a>.`
  }

  const reportErrorMessage = "There is a bug in ReadAi Meeting Scraper. Please report it at https://github.com/bhawani-nexus/readai-meeting-scraper/issues"
...
Promise.race([
  ...
  `
    ...
    activityStatus.style.cssText = `background-color: #32CD32; ${statusActivityCSS}`
  ...
  function showNotification(extensionStatusJSON) {
    ...
    text.style.cssText = "margin-top: 1rem; margin-bottom:1rem; font-size: medium"
      ...
      if (extensionStatusJSON.status === 200) {
        obj.style.cssText = `color: #32CD32; ${commonCSS}`      text.innerHTML = extensionStatusJSON.message
  ...
  function checkExtensionStatus() {
    return new Promise((resolve, reject) => {
      ...
      extensionStatusJSON = { status: 200, message: "<b>TranscripTonic is ready, enabling captions...</b> <br /> Please enable manually if not successful (More > Language > Captions)" }
      ...
      fetch(
        ...
        .then((result) => {
          ...
          if (!meetsMinVersion(chrome.runtime.getManifest().version, minVersion)) {
            ...
            extensionStatusJSON.message = `<strong>ReadAi Meeting Scraper is not running</strong> <br /> Please update to v${minVersion} by following <a href="https://github.com/bhawani-nexus/readai-meeting-scraper/wiki/Manually-update-ReadAi-Meeting-Scraper" target="_blank">these instructions</a>`
          }
          else {
