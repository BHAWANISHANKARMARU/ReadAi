...
function showNotification(extensionStatusJSON) {
  ...
  if (extensionStatusJSON.status === 200) {
    obj.style.cssText = `color: #32CD32; ${commonCSS}`
    ...
    fetch(
      ...
      .then((result) => {
        ...
        if (!meetsMinVersion(chrome.runtime.getManifest().version, minVersion)) {
          ...
          extensionStatusJSON.message = `<strong>ReadAi Meeting Scraper is not running</strong> <br /> Please force update to v${minVersion} by following <a href="https://github.com/bhawani-nexus/readai-meeting-scraper/wiki/Manually-update-ReadAi-Meeting-Scraper" target="_blank">these instructions</a>`
        }
        else {
