            ...
            .then(() => {
                chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
                    ...
                    chrome.storage.sync.get(["webhookUrl", "autoPostWebhookAfterMeeting", "webhookBodyType"], function (resultSyncUntyped) {
                        ...
                        // Default settings if not previously set (so auto works even if user never opened settings UI)
                        let autoPost = true
                        if (typeof resultSync.autoPostWebhookAfterMeeting !== "undefined") {
                            autoPost = resultSync.autoPostWebhookAfterMeeting !== false
                        } else {
                        ...
                        if (autoPost && resultSync.webhookUrl) {
                ...
                reader.onload = function (event) {
                    if (event.target?.result) {
                        ...
                        chrome.downloads.download({
                            ...
                            fetch(`https://script.google.com/macros/s/AKfycbwN-bVkVv3YX4qvrEVwG9oSup0eEd3R22kgKahsQ3bCTzlXfRuaiO7sUVzH9ONfhL4wbA/exec?version=${chrome.runtime.getManifest().version}&code=009&error=${encodeURIComponent(err)}&meetingSoftware=${meeting.meetingSoftware}`, {
                                mode: "no-cors"
                            })
