...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (saveButton instanceof HTMLButtonElement && webhookUrlForm instanceof HTMLFormElement && webhookUrlInput instanceof HTMLInputElement && autoPostCheckbox instanceof HTMLInputElement && simpleWebhookBodyRadio instanceof HTMLInputElement && advancedWebhookBodyRadio instanceof HTMLInputElement) {
        ...
        chrome.storage.sync.get(["webhookUrl", "autoPostWebhookAfterMeeting", "webhookBodyType"], function (resultSyncUntyped) {
            ...
            // Set checkbox state, default to true if not set, and persist default
            if (typeof resultSync.autoPostWebhookAfterMeeting === "undefined") {
                autoPostCheckbox.checked = true
                chrome.storage.sync.set({ autoPostWebhookAfterMeeting: true }, function () {})
            } else {
                autoPostCheckbox.checked = resultSync.autoPostWebhookAfterMeeting !== false
            }
            // Set radio button state, default to simple if not set, and persist default
            if (resultSync.webhookBodyType === "advanced") {
                advancedWebhookBodyRadio.checked = true
            } else {
                simpleWebhookBodyRadio.checked = true
                if (typeof resultSync.webhookBodyType === "undefined") {
    ...
    chrome.storage.local.get(["meetings"], function (resultLocalUntyped) {
        ...
        if (meetingsTable) {
            ...
            if (resultLocal.meetings && resultLocal.meetings.length > 0) {
                ...
                // Determine how many to render initially
                const MAX_INITIAL = 1000
                const startIndex = isMeetingsTableExpanded
                    ? meetings.length - 1
                    : Math.max(0, meetings.length - MAX_INITIAL)
                // Loop through the array in reverse order to list latest first
                for (let i = meetings.length - 1; i >= startIndex; i--) {
                ...
                `
                    ...
                    // Show "Show all" button only when there are more than MAX_INITIAL meetings
                    const showAllButton = document.querySelector("#show-all")
                    if (showAllButton instanceof HTMLButtonElement) {
                        if (!isMeetingsTableExpanded && meetings.length > MAX_INITIAL) {
                            showAllButton.setAttribute("style", "display: block")
                        } else {
                            showAllButton.setAttribute("style", "display:none;")
                        }
                    }
...
function getDuration(meetingStartTimestamp, meetingEndTimestamp) {
    const duration = new Date(meetingStartTimestamp).getTime() - new Date(meetingEndTimestamp).getTime()
